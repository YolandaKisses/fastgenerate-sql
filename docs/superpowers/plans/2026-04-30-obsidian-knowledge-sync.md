...
**Goal:** 为当前项目增加“同步到知识库”能力，通过 Python 后端异步调用 Hermes 生成数据源总览页与单表 Markdown，并把结果写入本地 Obsidian 目录。

**Architecture:** 前端在 Schema 管理页新增触发按钮和任务进度展示；后端新增知识库同步任务模型、接口和服务，复用 SQLite 中已有的 Schema 与补充备注作为输入；Hermes 仅负责生成增强说明，结构化字段明细由本地数据直接渲染到 Markdown。
...
- [ ] Step 3: 实现知识库任务创建、Hermes 调用、Markdown 渲染、Obsidian 写文件和状态更新。
...
- [ ] Step 2: 运行后端测试确认失败。
- [ ] Step 3: 实现 `POST /api/v1/schema/knowledge/sync/{datasource_id}` 和 `GET /api/v1/schema/knowledge/tasks/{task_id}`，并通过 FastAPI `BackgroundTasks` 触发执行。
...
- [ ] Step 1: 编写失败测试，覆盖“同步到知识库”按钮展示、点击后发起请求、轮询任务状态和展示进度。
- [ ] Step 2: 运行 `npm test -- tests/schema-knowledge-sync.spec.ts`，确认失败。
- [ ] Step 3: 最小实现前端按钮、禁用态、任务状态展示和轮询逻辑。
